<template>
  <div id="bachelorInfo">
    <div class="d-block d-mb-none pt-70">
      <div class="bachelorTitle w-100 d-flex align-items-center flex-row">
        <router-link class="pl-20" :to="{ name: 'bachelor' }">
          <img src="@/assets/images/icon/arrowLeft.png" alt="" />
        </router-link>
        <PhoneTitle title="學士班" :filterDate="false" />
      </div>
      <div class="w-100 newsInfo">
        <div class="px-20 pt-40 pb-90">
          <div class="w-100 newsInfo__card" v-for="item in bachelorMsg" :key="item.id">
  ...
  methods: {
    async getList() {
      await this.$api.departmentRule(this.listQuery).then((res) => {
        this.bachelorMsg = res.data.data.filter(
          (arr) => arr.id === this.$route.params.id
        );
        this.bachelorMsg[0].attachedFile = JSON.parse(
          this.bachelorMsg[0].attachedFile
        );
