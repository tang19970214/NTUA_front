<template>
  <div id="bachelor">
    <div class="web d-none d-mb-block">
      <div class="bachelorTable w-100 d-flex align-items-center flex-row cur-pointer" v-for="(item, index) in bachelorMsg" :key="'BT__' + index" @click="viewInfo(item)">
    ...
    <div class="phone d-block d-mb-none">
      <div class="w-100 newsInfo">
        <div class="px-20 pt-40 pb-90">
          <div class="w-100 newsInfo__card d-flex flex-row mb-15" v-for="(item, index1) in bachelorMsg" :key="index1" @click="goBachelorInfo(item)">
        ...
        <div class="p-40">
          <div class="w-100 pos-relative d-flex align-items-start justify-content-center flex-column">
            <div class="p-10 modal__content--tag">
              <span class="d-inline-flex">學士班</span>
            </div>
            ...
            <div class="w-100 mt-40 modal__content--link" v-if="selectNews.linkUrl">
...
export default {
  ...
  data() {
    return {
      listQuery: {
        TypeId: "SYS_DEPARTMENTRULE_Bachelor",
        page: 1,
        limit: 999,
        key: undefined,
      },
  ...
  methods: {
    ...
    goBachelorInfo(data) {
      this.$router.push({ name: "bachelorInfo", params: { id: data.id } });
    },
    showfileInfo(data) {
    ...
    async getList() {
      await this.$api.departmentRule(this.listQuery).then((res) => {
        this.bachelorMsg = res.data.data;
