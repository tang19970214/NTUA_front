<template>
  <div id="twoyearsInfo">
    <div class="d-block d-mb-none pt-70">
      <div class="twoyearsTitle w-100 d-flex align-items-center flex-row">
        <router-link class="pl-20" :to="{ name: 'twoyears' }">
          <img src="@/assets/images/icon/arrowLeft.png" alt="" />
        </router-link>
        <PhoneTitle title="二年制在職專班" :filterDate="false" />
      </div>
  ...
  methods: {
    async getList() {
      await this.$api.departmentRule(this.listQuery).then((res) => {
        this.twoyearsMsg = res.data.data.filter(
          (arr) => arr.id === this.$route.params.id
        );
        this.twoyearsMsg[0].attachedFile = JSON.parse(
          this.twoyearsMsg[0].attachedFile
        );
